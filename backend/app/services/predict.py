...
4. **套用现行官方公式**（issue ≥ 54 新公式，见 rank.py）：
       ...
       t = T[D_floor]，7 档阶梯（D0 周初 → D6 周末，单调上升）：
           T = [1.0527, 1.1381, 1.3900, 1.6061, 1.6900, 2.1574, 2.4700]
       D_floor = clamp(round((投稿时间 − 周期起点)/86400 − 0.5), 0, 6)
       （实现见 rank.time_correction，与官方周榜对拍口径一致；Δt = 投稿时间 − 周期起点）
