...
- 系数 `(view=1, favorite=15, like=3, coin=30)` 已用 112 期官方 API 全量验证为**完全准确**。
- `t` 为时间 / 新曲加成系数，**真实形态是「本周内投稿整天数 D_floor」的 7 档阶梯确定性函数**（`rank.time_correction`，2026-08-11 极限还原）：
  - 老曲（投稿早于本期起点）：`t = 1`
  - 新曲：`t = T[D_floor]`，`D_floor = clamp(round((投稿时间 − 本周起始快照时刻)/86400 − 0.5), 0, 6)`
  - 阶梯 `T = [1.0527, 1.1381, 1.3900, 1.6061, 1.6900, 2.1574, 2.4700]`（D0 周初 → D6 周末，单调上升），由官方 112 期周榜 JSON 的 `stats` 反推 `implied_t` 鲁棒估计得到，跨 pubtime 来源稳定；干净子集 score 相对误差中位≈0.38%。
...
> 系数已用 112 期官方 API 全量对拍验证。逐曲 `t` 无法仅凭 pubtime 精确反推（官方 t 主要给首次上榜歌曲加成），故复算得分为近似还原，榜单排名 100% 精确。

### 二次测算结论（2026-08-11）

- 系数与榜单排名**完全可复现**：新公式期 top20 重合率 **100%**。
- 逐曲 `score` 还原率 **68.8%**（|Δ|≤1）；根因为官方 `t` 并非 pubtime 简单函数，而是主要给「首次上榜」（`weeks_on_board==1`）歌曲的加成，幅度 `[1, 2.615]` 不固定。
- 钳制上限 `2.615` 来自官方 112 期 `implied_t` 实测（中位 1.0、p95 1.69、最大 2.615），消除脏数据导致的 `t=13.7 / 19.8` 等荒谬值。
