...
- `t` 为时间 / 新曲加成系数，**实现上钳制在 `[1.0, 2.615]`**（`rank.time_correction`，2026-08-11 二次测算修正）：
  - 老曲：`t = 1`；新曲：`t = log10(e^(Δt/86400/14)+1)+1` 后钳制到 `[1.0, 2.615]`
  - `Δt = 投稿时间 − 本周起始快照时刻（前一期统计截止，秒）`
  - `pubtime` 缺失或晚于本期起点 1 年以上（脏数据）按老曲 `t=1` 处理
