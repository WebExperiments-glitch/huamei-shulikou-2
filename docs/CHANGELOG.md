...
### 公式二次测算与校正（2026-08-11）
- 用户复核公式准确性：系数 `(view=1, favorite=15, like=3, coin=30)` **经验证完全准确**，榜单排名 **100% 可复现**（新公式期 top20 重合率 100%，排名直接用官方 `score`）。
- 逐曲 `score` 还原率 68.8%（|Δ|≤1）。根因：官方时间修正系数 `t` **并非 pubtime 的简单函数**，而是主要给「首次上榜」歌曲（`weeks_on_board==1`，占 boosted 样本的 84.6%）的加成，幅度在 `[1, 2.615]` 间不固定，无法仅凭 `pubtime` 精确反推。故复算得分为**近似还原**，榜单排名精确。
- 实施方案 A：把 `rank.time_correction` 的 `t` **钳制到 `[1.0, 2.615]`**（上限取自官方 112 期 `implied_t` 实测：中位 1.0、p95 1.69、最大 2.615），并对 `pubtime` 缺失 / 晚于本期起点 1 年以上（脏数据）按老曲 `t=1` 处理。消除脏数据导致的 `t=13.7 / 19.8` 等荒谬值，使「公式可视化实验室」展示的 t 与官方实际量级一致。
- 验证脚本见 `参考数据/`：`analyze_t_bounds.py`、`analyze_t_driver.py`、`verify_option_a.py`、之前的 `verify_new_formula_now.py`、`implied_t_check.py`、`公式测算报告.md`。

### 已知问题
- 网易云播放量接口已废弃，`play_count` 恒为 null
- 沙箱网络屏蔽 `music.163.com`，网易云工具在沙箱内返回降级提示
- 复算 `score` 为近似还原（官方 `t` 含不可反推的首登榜加成），榜单排名不受此影响（100% 精确）
