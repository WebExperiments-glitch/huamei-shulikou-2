...
enum RemoteAPI {
    ...
    static func url(_ path: String, settings: SettingsStore) -> URL? {
        settings.remoteBaseURL?.appendingPathComponent(path)
    }
    ...
    static func searchSongs(q: String = "", sort: String = "pubtime", order: String = "desc",
        ...
        guard let base = settings.remoteBaseURL else { return [] }
    ...
    static func songHistory(_ bvid: String, settings: SettingsStore) async -> [SongHistoryEntry] {
        ...
        do {
            ...
            let box = try await APIClient.shared.get("api/songs/\(bvid)/all-history", baseURL: base)
    ...
    static func hotSongs(sort: String = "score", limit: Int = 50, settings: SettingsStore) async -> [HeatItem] {
        ...
        do {
            ...
            let box = try await APIClient.shared.get("api/hot/songs?sort=\(sort)&limit=\(limit)", baseURL: base)
    ...
    static func hotMomentum(limit: Int = 20, settings: SettingsStore) async -> [HeatItem] {
        ...
        do {
            ...
            let box = try await APIClient.shared.get("api/hot/momentum?limit=\(limit)", baseURL: base)
            return box.items ?? []
        } catch {
    ...
    static func neteaseSearch(_ q: String, settings: SettingsStore) async -> [NeteaseSong] {
        ...
        do {
            ...
            let box = try await APIClient.shared.post("api/netease/search", body: body, baseURL: base)
            return box.items ?? box.songs ?? []
        } catch {
