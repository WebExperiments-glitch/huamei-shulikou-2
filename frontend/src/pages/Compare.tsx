    ...
    <>
      <Reveal>
        <PageHeader
          crumb="分析 · 多歌曲对比"
          title="歌曲对比"
          extra={<div style={{ display: "flex", gap: 8 }}><button className={`chip${metric === "rank" ? " active" : ""}`} onClick={() => setMetric("rank")}>排名轨迹</button><button className={`chip${metric === "score" ? " active" : ""}`} onClick={() => setMetric("score")}>得分轨迹</button></div>}
        />
      </Reveal>
